...
const AllTasksSection = ({ user }) => {
  ...
  // organize select box>>> All tasks, complete, notcomplete
  ...
  if (value) {
    return (
      <div>
        ...
        >
          {/* Organize when remove oldes + news */}
          {selectValue === "aaa" && (
            <div>
              <button
              ...
              >
                ...
                {i18n.language === "fr" && "la plus récente "}
              </button>
              <button
              ...
              >
                ...
                {i18n.language === "fr" && "la plus ancien "}
              </button>
            </div>
          )}
          <select
          ...
          >
            <option value="aaa">
              ...
              {i18n.language === "en" && "All Tasks First"}
            ...
            <option value="ccc">
              ...
              {i18n.language === "ar" && "المهام الغير المكتملة"}
          ...
          {value.docs.map((item) => {
            return (
              <article key={item.data().id} dir="auto" className="one-task">
                <Link
                  className="task-link"
                  to={`/edit-task/editTask/${item.data().id}`}
                >
                  ...
                  <p className="time">
                    <Moment className="time" fromNow date={item.data().id} />
                  </p>
                </Link>
              </article>
            );
